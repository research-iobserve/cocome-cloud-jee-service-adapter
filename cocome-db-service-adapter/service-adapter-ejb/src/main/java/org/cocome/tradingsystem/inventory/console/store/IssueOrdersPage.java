...
final class IssueOrdersPage
	...
	private static final Logger __log__ = Logger.getLogger(StoreConsole.class.getName());
		...
		return new EditableColumnHandler<ProductSupplierAmountHolder>("Amount to Order") {
			...
			boolean setValue(
				...
				try {
					...
					__log__.info("Invalid product amount!"+ e.getMessage());
	...
	private List<ComplexOrderTO> __orderProducts() {
		...
		__log__.info("Number of order entries: " + orderEntryCount);
		...
		if (orderEntryCount > 0) {
			...
			__log__.info("Number of issued orders: " + issuedOrderCount);
	...
	private List<ComplexOrderTO> __orderProducts(final ComplexOrderTO order) {
		try {
			...
			__log__.severe("Failed to order products!"+ re.getMessage());
