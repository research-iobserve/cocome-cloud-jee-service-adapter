...
public final class EnterpriseQueryProvider implements IEnterpriseQuery {
	...
	@Override
	public List<TradingEnterprise> queryAllEnterprises(IPersistenceContext pctx) {
		final EntityManager em = __getEntityManager(pctx);
		TypedQuery<TradingEnterprise> query = 
									em.createQuery("SELECT te FROM TradingEnterprise te",
