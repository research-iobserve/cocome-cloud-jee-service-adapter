...
public final class CashDeskModel
	...
	private static final Logger LOG =
			Logger.getLogger(CashDeskModel.class.getName());
	...
	void addItemToSale(final long barcode) throws JMSException {
		...
		if (this.canAcceptNextItem()) {
			...
			try {
				...
				LOG.severe("Failed to communicate with store inventory: " + re.getMessage());
			}
	...
	void finishCreditCardPayment(final int pin)
		...
		try {
			...
			LOG.severe("Could not connect to the bank: " + cnae.getMessage());
		} catch (final RemoteException re) {
			LOG.severe("Failed to communicate with the bank: " + re.getMessage());
		}
	}
	...
	void finishCreditCardPayment(final int pin, final IBank bank)
		...
		if (debitResult == DebitResult.OK) {
			...
			LOG.severe("Unexpected debit result " + debitResult);
		}
	}
